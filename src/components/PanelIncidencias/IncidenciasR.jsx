...
function IncidenciasR() {
  ...
  useEffect(() => {
    const fetchResueltas = async () => {
      try {
        const response = await fetch(`${API_LINK}/incidencia/resueltas`);
        const data = await response.json();
        if (data.incidencias) {
          setResueltas(data.incidencias);
        }
      } catch (error) {
        console.error("Error fetching incidencias resueltas:", error);
      }
    };
    fetchResueltas();
  }, [API_LINK]);

  // Ícono inline (igual que en IncidenciasP)
  function BusIcon() {
    return (
