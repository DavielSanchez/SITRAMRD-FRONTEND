...
function IncidenciasP() {
  ...
  useEffect(() => {
    const fetchPendientes = async () => {
      try {
        const response = await fetch(`${API_LINK}/incidencia/pendientes`);
        const data = await response.json();
        if (data.incidencias) {
          setPendientes(data.incidencias);
        }
      } catch (error) {
  ...
  // Función inline para el ícono (se usa el mismo estilo que en el código original)
  function BusIcon() {
    return (
  ...
  return (
    <div className="bg-[#f1f1ff] shadow-md rounded-lg p-4 flex flex-row items-center gap-3">
      ...
      <span className="text-[#6a62dc] text-xl font-bold">
        Incidencias pendientes ({pendientes.length})
      </span>
    </div>
  );
}

export default IncidenciasP;
